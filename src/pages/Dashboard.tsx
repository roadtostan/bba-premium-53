...
import { getReportsByUser, getPendingActionReports, canCreateNewReport } from '@/lib/data';
...
export default function Dashboard() {
  ...
  const [reports, setReports] = useState<Report[]>(() => user ? getReportsByUser(user.id) : []);
  const [pendingActionReports, setPendingActionReports] = useState<Report[]>(() => 
    user ? getPendingActionReports(user.id) : []
  );
  ...
  const filteredReports = () => {
    ...
    return reports.filter(report => {
      switch (activeTab) {
        case 'pending':
          return report.status === 'pending_subdistrict' || report.status === 'pending_city';
        case 'approved':
  ...
  const handleApprove = (reportId: string) => {
    ...
    if (reportIndex !== -1) {
      ...
      if (user.role === 'subdistrict_admin' && report.status === 'pending_subdistrict') {
        ...
        toast.success(`Report "${report.title}" approved and sent to City Admin`);
      ...
      setPendingActionReports(pendingActionReports.filter(r => r.id !== reportId));
    }
  };
  ...
  const handleReject = (reportId: string) => {
    ...
    if (reportIndex !== -1) {
      ...
      updatedReports[reportIndex].rejectionReason = reason || 'No reason provided';
      
      setReports(updatedReports);
      setPendingActionReports(pendingActionReports.filter(r => r.id !== reportId));
      
      toast.info(`Report "${updatedReports[reportIndex].title}" has been rejected`);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 animate-fadeIn">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {(user.role === 'subdistrict_admin' || user.role === 'city_admin') && pendingActionReports.length > 0 && (
          <div className="mb-8">
        ...
        {user.role === 'branch_user' && !canCreate && (
          <div className="mb-8 p-4 border rounded-lg bg-yellow-50 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-200">
            <p className="text-sm">
              You have pending reports awaiting approval. You can create a new report once all pending reports are processed.
            </p>
          </div>
        )}
          ...
          <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab} className="w-full">
            <div className="border-b mb-4">
              <TabsList className="w-full justify-start">
                ...
                {user.role === 'branch_user' && (
                  <TabsTrigger value="draft" className="data-[state=active]:border-b-2 data-[state=active]:border-primary">
                    Drafts
                  </TabsTrigger>
                )}
                ...
                <TabsTrigger value="approved" className="data-[state=active]:border-b-2 data-[state=active]:border-primary">
                  Approved
                </TabsTrigger>
                <TabsTrigger value="rejected" className="data-[state=active]:border-b-2 data-[state=active]:border-primary">
                  Rejected
                </TabsTrigger>
              </TabsList>
            </div>
